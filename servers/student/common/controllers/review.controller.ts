...
class ReviewController implements IController {
    ...
    constructor() {
        ...
        this.router.post(
            "/comment/:reviewID?",
            DTOValidation.extractParams(["reviewID"]),
            AuthController.protect,
            catchAsync(this.putComment)
        );
        this.router.get(
            "/view/:reviewID?",
            AuthController.protect,
            catchAsync(this.getReviewById)
        );

        this.router.post(
            "/:classID?",
            DTOValidation.validate<ReviewDTO>(ReviewDTO),
            AuthController.protect,
            catchAsync(this.createReview)
        );
        this.router.get(
            "/:classID?",
            AuthController.protect,
            catchAsync(this.getReviews)
        );
    }
    ...
    private async putComment(req: Request, res: Response, next: NextFunction) {
        ...
        const review: any = await ReviewModel.findById(
            req.body.reviewID
        ).populate("class joinedInfo");
        if (!review) {
            return next(new AppError("The review not found", 404));
        }
        ...
        const commentsInfo = await ReviewModel.findById(
            req.body.reviewID
        ...
        const newNotification = new NotificationModel({
            ...
            navigation: `/classes/reviews/${
                review.class.slug
            }/view/${review._id.toString()}`,
            formatedDate: getFormatDateTime(),
        ...
        const notification = await NotificationModel.findById(createdData._id)
            .populate("user class")
            .lean();
    ...
    private async getReviewById(
        ...
        const joinedInfo = await JoinedClassInfoModel.findById(
            review.joinedInfo._id
        )
            .populate("user")
            .lean();
    ...
    private async getReviews(req: Request, res: Response, next: NextFunction) {
        ...
        const joinedClassInfo = await JoinedClassInfoModel.findOne({
    ...
    private async createReview(
        ...
        if (reviewExist) {
            return next(
                new AppError(
                    `You have already reviewed ${reviewInfo.composition} composition!`,
                    403
                )
            );
        }
