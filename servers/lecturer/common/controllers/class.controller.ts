...
class ClassController implements IController {
  ...
  private getClass = async (req: Request, res: Response, next: NextFunction) => {
    if (req.body?.id) {
        ...
        { $unwind: "$owner" },
        {
          $match: {
            $and: [
              ...
              {
                $or: [{ "lecturers._id": new mongoose.Types.ObjectId(req.user?.id) }, { "owner._id": new mongoose.Types.ObjectId(req.user?.id) }],
              },
            ],
          },
        },
      ...
      const students = await JoinedClassInfoModel.find({
        class: classInfo._id,
        user: {
      ...
      },
      {
        $match: {
          $and: [
            ...
            {
              $or: [{ "lecturers._id": new mongoose.Types.ObjectId(req.user?.id) }, { owner: new mongoose.Types.ObjectId(req.user?.id) }],
            },
          ],
        },
      },
