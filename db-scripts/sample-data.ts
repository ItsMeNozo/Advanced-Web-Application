...
const joinedClassInfos = JSON.parse(fs.readFileSync(`${__dirname}/data/joined_class_infos.json`, "utf8"));
...
// DELETE Notifications data FROM DB
const deleteNotificationData = async () => {
  try {
    await NotificationModel.deleteMany();
    console.log("Deleted nofication info data successfully!");
  } catch (err) {
    console.log(err);
  }
};
...
// DELETE JOINED CLASS INFO DATA FROM DB
const deleteReviewInfoData = async () => {
  try {
    await ReviewModel.deleteMany();
    console.log("Deleted review info data successfully!");
  } catch (err) {
    console.log(err);
  }
};
